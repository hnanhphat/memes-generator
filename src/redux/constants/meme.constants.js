// GET ALL MEMES
export const GET_MEMES_REQUEST = "MEME.GET_MEMES_REQUEST";
export const GET_MEMES_SUCCESS = "MEME.GET_MEMES_SUCCESS";
export const GET_MEMES_FAILURE = "MEME.GET_MEMES_FAILURE";

// GET SINGLE MEME
export const GET_SINGLE_MEME_REQUEST = "MEME.GET_SINGLE_MEME_REQUEST";
export const GET_SINGLE_MEME_SUCCESS = "MEME.GET_SINGLE_MEME_SUCCESS";
export const GET_SINGLE_MEME_FAILURE = "MEME.GET_SINGLE_MEME_FAILURE";

// CREATE MEME
export const CREATE_MEME_REQUEST = "MEME.CREATE_MEME_REQUEST";
export const CREATE_MEME_SUCCESS = "MEME.CREATE_MEME_SUCCESS";
export const CREATE_MEME_FAILURE = "MEME.CREATE_MEME_FAILURE";

// UPLOAD MEME
export const UPDATE_MEME_REQUEST = "MEME.UPDATE_MEME_REQUEST";
export const UPDATE_MEME_SUCCESS = "MEME.UPDATE_MEME_SUCCESS";
export const UPDATE_MEME_FAILURE = "MEME.UPDATE_MEME_FAILURE";

// DELETE MEME
export const DELETE_MEME_REQUEST = "MEME.DELETE_MEME_REQUEST";
export const DELETE_MEME_SUCCESS = "MEME.DELETE_MEME_SUCCESS";
export const DELETE_MEME_FAILURE = "MEME.DELETE_MEME_FAILURE";

// SELECTED MEME
export const SET_SELECTED_MEME = "MEME.SET_SELECTED_MEME";
